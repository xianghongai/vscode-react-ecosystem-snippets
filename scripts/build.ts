 ...
 *
 * 若干能力提供了多个同类库的片段（客户端状态 Zustand / Jotai，通用 Hooks
 * ahooks / react-use / usehooks-ts），使用方只会选其中一个。自建只含所选库的
 * 变体时把排除项写进 `.env`（由 dotenv 加载，该文件已被 git 忽略），源码因此保持不变：
 *
 *     SNIPPETS_EXCLUDE=src/ahooks/**,src/react-use/**
 *
 * 也可只对单次构建生效：`SNIPPETS_EXCLUDE=... pnpm run build`，此时命令行传入
 * 的值优先于 `.env`。未设置时包含全部来源，这是发布版的形态。
 *
 * 必须用 `globSync` 的 `exclude` 选项：否定 glob（`'!src/ahooks/**'`）无效，
 * 会被当作普通模式处理。
 */
